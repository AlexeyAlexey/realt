...
class Realt < ActiveRecord::Base
  ...
  attr_accessor :email, :xml
		 ...
		 if !(arrayTd.empty?)
		   then
		     ...
		     hashTd.each_key {|key| hashTd[key] = arrayTd[j]; j += 1} #хэш с данными	
			  ...
			  @xml.addTable arrayTd     #передаем массив значений столбцов
		 end
   ...
   def catchPage(*titleTableExpr) 
     ...
     @xml = FileXml.new                 #создаем таблицу XML(HTML) class FileXml
     @xml.newTable @countColumn #создаем таблицу XML(HTML) указывая количество столбцов class FileXml
     @xml.addTable titleTable
